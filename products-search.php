<?php
	...
	$select="select p_id,image1,name,short_description,price,bulk_price FROM product WHERE p_id!='' ";
	...
	$select.="order by bulk_price ASC";
...
$(document).ready(function() {
	...
	var pricetype = $('#pricetypeval').val();
	var price = $('#priceval').val();
	$('#results').load("autoload_process_search.php", {'group_no':track_load,'pricetype':pricetype,'price':price}, function() {track_load++;});
	$(window).scroll(function() {
		if($(window).scrollTop() + $(window).height() == $(document).height())
		{
			if(track_load <= total_groups && loading==false) 
			{
				...
				$.post('autoload_process_search.php',{'group_no': track_load,'pricetype':pricetype,'price':price}, function(data){				
					$("#results").append(data); 
...
<script type="text/javascript">
		...
		function findproduct(priceval){
			...
			var price=priceval;
			$('#results').load("autoload_process_search.php", {'group_no':track_load,'cid':cid,'scid':scid,'sscid':sscid,'price':price}, function() {track_load++;});
			$(window).scroll(function() {
				if($(window).scrollTop() + $(window).height() == $(document).height())
				{
					if(track_load <= total_groups && loading==false) 
					{
						loading = true;
						$('.animation_image').show();
						$.post('autoload_process_search.php',{'group_no': track_load,'cid':cid,'scid':scid,'sscid':sscid,'price':price}, function(data){				
							$("#results").append(data); 
							$('.animation_image').hide();
							track_load++; 
							loading = false; 
...
<div class="main">
 ...
 <div class="middle_main_new">
  <div class="middle_main">
    ...
    <div class="nursery">
      <?php /*?><div class="nursery_left">
	    <?php
				$select_menu_allnc="select * from categories where id='1' order by id ASC";
            ...
            <nav>
              <ul id="nav">
                ...
                <li><a href="#"><?php echo $fetch_sub_menu_allnc['name'];?></a>
                  <ul>
				    <?php
						$select_sub_sub_menu_allnc="select * from subsubcategories where parent_id='1' and sub_parent_id='".$fetch_sub_menu_allnc['id']."' and status='1' order by id ASC";
      ...
      <div class="nursery_right2">
        <div class="nurserymain_product2">Product</div>
        <div class="img_panel" id="results"></div>
        <div class="animation_image" style="display:none" align="center"><img src="http://honeybeesgift.com/admin/images/ajax-loader.gif"></div>
      </div>
