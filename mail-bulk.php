     ...
     <td><table width='100%' border='0' style='border:1px solid #005387; font-size:12px; text-align:center;'>
        <tr bgcolor='#005387'>
		  ...
		  while($fetch_data=mysqli_fetch_array($query_orderdetail))
		  {
		   ...
		   //$query_product=mysqli_query($conn,"select image from tempcart where session_id='".$session_id."' order by id");
		   $sql = "select image from tempcart where id='".$fetch_data['tempcartid']."'";
		  // echo $sql."<BR>";
