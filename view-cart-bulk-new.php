<?php
	...
	if(isset($_POST['placeorder'])){
		header("Location:order-place-bulk-new.php");
		exit();
	}
	...
	if(isset($_POST['pro_id']))
	{
		...
		if(($_POST['newimg']!="") || ($_POST['newqnt']!="") || ($_POST['newallqnt']))
		{
			...
			$get=mysql_fetch_array(mysql_query("select * from product where p_id='".$product_id."'"));
			$sql_result=mysql_query("select * from tempcart where product_id='".$product_id."' and session_id='".$session_id."'");
			...
			if($rows==0)
			{
				...
				for($i=0;$i<count($expnewcurqnt)-1;$i++)
				{
					...
					mysql_query("insert into tempcart set session_id='".mysql_real_escape_string($session_id)."',product_id='".mysql_real_escape_string($get['p_id'])."',product_title='".$ppp[$i]."',price='".mysql_real_escape_string($totalprice)."',unit_price='".mysql_real_escape_string($get['bulk_price'])."',quantity='".mysql_real_escape_string($expnewcurqnt[$i])."',image='".mysql_real_escape_string($expnewcurimg[$i])."',personalization='".mysql_real_escape_string($expnewcurallqnt[$i])."',product_code='".mysql_real_escape_string($get['product_code'])."'");
			  }
			}
		}
		if(($_POST['qnt']!="") || ($_POST['allqnt']!=""))
		{
			...
			for($i = 1; $i < $_POST['qnt']; $i++)
			{
				$temp_img_name = "img". $i;
				$img_new = $_FILES[$temp_img_name]['name'];
				move_uploaded_file($_FILES[$temp_img_name]['tmp_name'], 'admin/adds/'.$img_new);
				$image_text .= $img_new.",";
			}
			$allqnt=$_POST['allqnt'];
			if($allqnt!="")
			...
			if($rows==0)
			{
				...
				mysql_query("insert into tempcart set session_id='".mysql_real_escape_string($session_id)."',product_id='".mysql_real_escape_string($get['p_id'])."',product_title='".mysql_real_escape_string($get['name'])."',price='".mysql_real_escape_string($totalprice)."',unit_price='".mysql_real_escape_string($get['bulk_price'])."',quantity='".mysql_real_escape_string($qnt)."',image='".mysql_real_escape_string($image_text)."',personalization='".mysql_real_escape_string($curallqnt)."',product_code='".mysql_real_escape_string($get['product_code'])."'");
			}
		}
		header("location:view-cart-bulk-new.php");
		exit();
	}
	if($_GET['mode']=='delete')
	{	
		mysql_query("delete from tempcart where id='".$_GET['id']."' and session_id='".$session_id."'");
		header("location:view-cart-bulk-new.php");
		exit();
	} 
	...
	$(document).ready(function(){
	  $("input[id^='id_']").each(function() {
	   ...
	   $("#qty"+id).blur(function(event){
			 ...
			 function(data){
			 	$("#totalamt").html(data);
				window.location="view-cart-bulk-new.php";
			 });
		});
	   });
	});
...
<div class="wecart_bag">
  ...
  <?php 
	  if($count_record>0){	  
	      $i=0;
		  while($temp_data= mysql_fetch_array($sql_query)){
			$tmp_id[]=$temp_data['product_id'];
			if($temp_data['image']!=""){
				$temp_img_name = split(",",$temp_data['image']);
				$image='admin/adds/'.$temp_img_name[0];
				//echo $image;
			}
			else{
				$query_image="select image1,color from product where p_id='".$temp_data['product_id']."'";
				$sql_image=mysql_query($query_image);
				$data_image=mysql_fetch_array($sql_image);
  ...
  <div class="item_description">
    ...
    <div class="item_description_right">
      ...
      <p><a href="view-cart-bulk-new.php?id=<?php echo $temp_data['id'];?>&mode=delete" style="margin-left:10px;" onclick="return confirm('are you sure?');"><img src="images/delete-icon.png" width="22" height="22" /></a></p>
      </div>
    </div>
    
<div class="item_description_images" style="border-bottom:none;">
  <?for($i = 1; $i < (sizeof($temp_img_name)-1); $i++){?>
  <div class="item_images"><img src="admin/adds/<?=$temp_img_name[$i]?>" width="86" height="76" /></div>
  <?}?>
  </div>      
