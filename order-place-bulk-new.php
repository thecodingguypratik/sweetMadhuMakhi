<?php
	...
	$sql_query_curr=mysql_query("select * from tempcart where session_id='".$session_id."' order by id");
	while($temp_data_curr=mysql_fetch_array($sql_query_curr)){
	...
	if(isset($_POST['checkout'])){
		...
		$oid=mysql_insert_id();
		//insert order detail
		$sql_query_currn=mysql_query("select * from tempcart where session_id='".$session_id."' order by id");
		while($fetch_order_detailn=mysql_fetch_array($sql_query_currn)){
			$query_order_detail="insert into `order_detail` set o_id='".$oid."',product_id='".$fetch_order_detailn['product_id']."',product_title='".$fetch_order_detailn['product_title']."',product_code='".$fetch_order_detailn['product_code']."',quantity='".$fetch_order_detailn['quantity']."',price='".$fetch_order_detailn['price']."',unit_price='".$fetch_order_detailn['unit_price']."',size='".$fetch_order_detailn['size']."',personalization='".$fetch_order_detailn['personalization']."',image='".$fetch_order_detailn['image']."',item_status='1'";
			mysql_query($query_order_detail);
		}
		//insert payment detail
		$query_order_payment="insert into `order_payment` set o_id='".$oid."',payment_type='".$_POST['paytype']."'";
		mysql_query($query_order_payment);
		header("Location:thanks-new.php?order_no=".$orderno."&o_id=".$oid);
		exit();
    ...
    <div class="biling_information">Billing Information</div>
      <div class="billing_email">
        ...
        <div class="billing_email_right">
          <input name="fname" id="fname" type="text"  class="billing_formbox"/>
        </div>
      </div>
      <div class="billing_email">
        <div class="billing_email_left">Last Name*</div>
        <div class="billing_email_right">
          <input name="lname" id="lname" type="text"  class="billing_formbox"/>
        </div>
      </div>
      <div class="billing_email">
        <div class="billing_email_left">Email Address*</div>
        <div class="billing_email_right">
          <input name="email" id="email" type="text"  class="billing_formbox"/>
        </div>
      </div>
      <div class="billing_email">
        ...
        <div class="billing_email_right">
          <select name="state" id="state" class="country_formbox">
            ...
            <?php
                ...
                while($fetch_state=mysql_fetch_array($result_state)){
            ?>
            <option value="<?php echo $fetch_state['sid'];?>"><?php echo $fetch_state['State'];?></option>
            <?php
                }
            ?>
          </select>
      ...
      <div class="billing_email">
        ...
        <div class="billing_email_right">
          <input name="address" id="address" type="text"  class="billing_formbox"/>
        </div>
      </div>
      <div class="billing_email">
        <div class="billing_email_left">Landmark*</div>
        <div class="billing_email_right">
          <input name="landmark" id="landmark" type="text"  class="billing_formbox"/>
        </div>
      </div>
      <div class="billing_email">
        <div class="billing_email_left">Zip/Postal code*</div>
        <div class="billing_email_right">
          <input name="pincode" id="pincode" type="text"  class="billing_formbox"/>
        </div>
      </div>
      <div class="billing_email">
        <div class="billing_email_left">Mobile*</div>
        <div class="billing_email_right">
          <input name="mobile" id="mobile" type="text"  class="billing_formbox"/>
        </div>
      </div>
  </div>
  ...
  <div class="main_free_shipping" >
    <div class="payment_method">
      ...
      <div class="payment_billing_email_right" style="margin-right:5px;text-wrap:none;">
		...
		<div class="item_description_images" style="border-bottom:none; width:480px;">
  <?for($i = 1; $i < (sizeof($temp_img_name)-1); $i++){?>
  <div class="item_images" style="padding-bottom:10px;"><img src="admin/adds/<?=$temp_img_name[$i]?>" width="86" height="76" /></div>
  <?}?>
  </div>      
