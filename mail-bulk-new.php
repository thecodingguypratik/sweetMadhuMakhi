     ...
     <td><table width='100%' border='0' style='border:1px solid #005387; font-size:12px; text-align:center;'>
        <tr bgcolor='#005387'>
		  ...
		  while($fetch_data=mysql_fetch_array($query_orderdetail))
		  {
		   ...
		   {
			  $temp_img_name = split(",",$fetch_product['image']);
			  $image='admin/adds/'.$temp_img_name[0];
			//	$fetch_product['image'];
			  for($i = 0; $i < (sizeof($temp_img_name)-1); $i++){
			  $message.="<a href='http://honeybeesgift.com/admin/adds/".$temp_img_name[$i]."'><img src='http://honeybeesgift.com/admin/adds/".$temp_img_name[$i]."' width='50' height='70'/></a><BR>";
			  }
		   }
      $message.="</td>
			</tr></table></td>
