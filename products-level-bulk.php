<?php
	...
	$select="select p_id,product_code,image1,name,short_description,price,bulk_price FROM product WHERE cat_id='".$_REQUEST['cid']."' and sub_cat_id='".$_REQUEST['scid']."' and sub_sub_cat_id='".$_REQUEST['sscid']."'";
	if($min!="" && $max!=""){
...
$(document).ready(function() {
	...
	$('#results').load("autoload_process_level_bulk.php", {'group_no':track_load,'cid':cid,'scid':scid,'price':price}, function() {track_load++;});
	$(window).scroll(function() {
		if($(window).scrollTop() + $(window).height() == $(document).height())
		{
			if(track_load <= total_groups && loading==false) 
			{
				...
				$.post('autoload_process_level_bulk.php',{'group_no': track_load,'cid':cid,'scid':scid,'price':price}, function(data){				
					$("#results").append(data); 
