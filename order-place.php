...
?>
    <div class="biling_information">
        <a href="javascript:void(0)" onClick="document.getElementById('lights_cor1').style.display='block'">Please Login</a>
        <br />
        <br />
    </div>
<?php }else{ ?>
        ...
        <div class="main_customer_information">
          ...
          <select name="city" id="city" class="country_formbox">
			...
			if($row_reg['state'] != "")
			{
                ...
                while($fetch_state=mysqli_fetch_array($result_state)){
            ?>
            <option value="<?php echo $fetch_state['ctid'];?>" <?php if($fetch_state['ctid'] == $row_reg['city']) echo "selected";?>><?php echo $fetch_state['cname'];?></option>
            <?php
                }
			}
            ?>
