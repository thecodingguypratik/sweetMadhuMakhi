<?php
	...
	$select="select p_id,product_code,image1,name,short_description,price,actual_price,bulk_price FROM product_new WHERE p_id!='' ";
	if($min!="" && $max!=""){
...
$(document).ready(function() {
	...
	$('#results').load("autoload_process_season.php", {'group_no':track_load,'price':price}, function() {track_load++;});
	$(window).scroll(function() {
		if($(window).scrollTop() + $(window).height() == $(document).height())
		{
			if(track_load <= total_groups && loading==false) 
			{
				...
				$.post('autoload_process_season.php',{'group_no': track_load,'price':price}, function(data){				
					$("#results").append(data); 
