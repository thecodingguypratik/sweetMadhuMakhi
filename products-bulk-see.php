...
$(document).ready(function() {
	...
	$('#results').load("autoload_process_bulk.php", {'group_no':track_load,'cid':cid,'price':price}, function() {track_load++;});
	$(window).scroll(function() {
		if($(window).scrollTop() + $(window).height() == $(document).height())
		{
			if(track_load <= total_groups && loading==false) 
			{
				...
				$.post('autoload_process_bulk.php',{'group_no': track_load,'cid':cid,'price':price}, function(data){				
					$("#results").append(data); 
...
<script type="text/javascript">
		...
		function findproduct(priceval){
			...
			var cid = <?php echo $_REQUEST['cid'];?>;
			var price=priceval;
			$('#results').load("autoload_process_bulk-see.php", {'group_no':track_load,'cid':cid,'price':price}, function() {track_load++;});
			$(window).scroll(function() {
				if($(window).scrollTop() + $(window).height() == $(document).height())
				{
					if(track_load <= total_groups && loading==false) 
